import paul from "/public/images/team/paul.jpg";
import robert from "/public/images/team/robert.jpg";
import rosen from "/public/images/team/rosen.jpg";
import ryan from "/public/images/team/ryan.jpg";
import brent from "/public/images/team/Brent.jpg";

const teamData = [
  {
    id: "37^#j",
    name: "Brent Watkins",
    avt: brent,
    desic: "Founder & CEO, with over 20 years in the real estate sector, has an extensive background in property development, investment, and digital innovation. ",
    profileLink: "https://www.linkedin.com/in/bwmwre/",
  },
  // {
  //   id: "7^#j",
  //   name: "Ryan Jackson",
  //   avt: ryan,
  //   desic: "Head of Business, a seasoned professional with over 15 years of experience in real estate operations and management. ",
  //   profileLink: "/",
  // },
  // {
  //   id: "7er^#j",
  //   name: "Brittany Rosen",
  //   avt: rosen,
  //   desic: "VP, Finance and Strategy, a strategic marketing leader, brings over a decade of experience in real estate marketing and branding.",
  //   profileLink: "/",
  // },
  // {
  //   id: "7e3^#j",
  //   name: "Robert Henriks",
  //   avt: robert,
  //   desic: "Senior Developer, a tech enthusiast with a passion for creating impactful digital solutions, has over 8 years of experience in software development, with a focus on real estate applications.",
  //   profileLink: "/",
  // },
];

export default teamData;
